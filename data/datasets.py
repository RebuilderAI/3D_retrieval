...
class Objaverse_lvis_openshape(data.Dataset):
    def __init__(self, config):
        ...
        for root, dirs, files in os.walk('./preprocessed'):
            if len(dirs) < 1:
                pre_files.extend(files)

        pre_files = list(map(lambda x: x.split('.')[0], pre_files))  
        
        # self.file_list = self.file_list[:100]
        
        test_index = []
        for idx, file_info in enumerate(self.file_list):
            if file_info['point_path'].split('/')[-1].split('.')[0] in pre_files:
                test_index.append(idx)
                
        self.file_list = [self.file_list[i] for i in test_index]
