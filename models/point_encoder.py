...
def fps(data, number):
    ...
    fps_data = []
    for batch in data:
        pcd = o3d.t.geometry.PointCloud(o3d.core.Tensor(batch))
        fps_batch = pcd.farthest_point_down_sample(number).point.positions
        fps_batch = torch.from_numpy(fps_batch.numpy()).unsqueeze(0)
        fps_data.append(fps_batch)
    fps_data = torch.cat(fps_data, dim=0).cuda()
    return fps_data
